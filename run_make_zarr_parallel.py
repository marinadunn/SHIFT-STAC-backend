...
def make_slurm_script(username, folder_name, dataset_date,
    ...
    # Alternatively to sbatch, can use 'salloc', an interactive batch for monitoring memory
        ...
        outfile.write(f'#salloc --account=s3673\n')
        outfile.write(f'#salloc --mem-per-cpu=2048\n')
        outfile.write(f'#salloc --time=3:00:00\n')
        outfile.write(f'#salloc --constraint="sky|hasw"\n')
